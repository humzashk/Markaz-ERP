...
router.get('/add', wrap(async (req, res) => {
  const orders   = (await pool.query(`SELECT o.id, o.order_no, o.bilty_no, c.name AS customer_name FROM orders o JOIN customers c ON c.id=o.customer_id WHERE o.bilty_no IS NOT NULL AND o.bilty_no <> '' ORDER BY o.id DESC`)).rows;
  const invoices = (await pool.query(`SELECT i.id, i.invoice_no, i.bilty_no, c.name AS customer_name FROM invoices i JOIN customers c ON c.id=i.customer_id WHERE i.bilty_no IS NOT NULL AND i.bilty_no <> '' ORDER BY i.id DESC`)).rows;
...
router.get('/api/search', wrap(async (req, res) => {
  ...
  const orders   = (await pool.query(`SELECT o.id, o.order_no AS ref, 'order' AS type, o.bilty_no, c.name AS party FROM orders o JOIN customers c ON c.id=o.customer_id WHERE o.bilty_no IS NOT NULL AND o.bilty_no <> '' AND (o.order_no ILIKE $1 OR o.bilty_no ILIKE $1 OR c.name ILIKE $1) ORDER BY o.id DESC LIMIT 20`, [q])).rows;
  const invoices = (await pool.query(`SELECT i.id, i.invoice_no AS ref, 'invoice' AS type, i.bilty_no, c.name AS party FROM invoices i JOIN customers c ON c.id=i.customer_id WHERE i.bilty_no IS NOT NULL AND i.bilty_no <> '' AND (i.invoice_no ILIKE $1 OR i.bilty_no ILIKE $1 OR c.name ILIKE $1) ORDER BY i.id DESC LIMIT 20`, [q])).rows;
...
router.get('/view/:id', wrap(async (req, res) => {
  ...
  const bilty = (await pool.query(`
    SELECT b.*, o.order_no, i.invoice_no, COALESCE(c.name, c2.name, '') AS customer_name
    FROM bilty b
    LEFT JOIN orders o   ON o.id  = b.order_id
    LEFT JOIN invoices i ON i.id  = b.invoice_id
    LEFT JOIN customers c  ON c.id = o.customer_id
